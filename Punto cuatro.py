...
#Hallando información de días en los que hay alerta de alertas
alertAlertass=(data.get("alertAlertas"))#Obteniendo diccionario alertAlertas dentro del diccionario
for k,v in alertAlertass.items():#Recorriendo pares llave:valor
    if v=="X":# Si el valor es X en el día
          fecha=data.get("dt")#Obteniendo diccionario "dt" dentro del diccionario
          dia=fecha[k]#Hallando la fecha del día en el que v es X (es decir el día en el que sí hay alerta)
          print((datetime.utcfromtimestamp(dia).strftime('%Y-%m-%d %H:%M:%S'))+ " Alerta alertas")#Se imprime la fecha de la alerta y el tipo de alerta

#Hallando información de días en los que hay alerta de precipitación
alrtaPrecipitaciones=data.get("alertPrecip")#Obteniendo diccionario alertPrecip dentro del diccionario
precipitacion=(data.get("prcp"))#Obteniendo diccionario prcp dentro del diccionario
for k,v in alrtaPrecipitaciones.items():#Recorriendo pares llave:valor
    if v=="X":#Si el valor es X en el día
        fecha=data.get("dt")# Obteniendo diccionario "dt" dentro del diccionario
        dia=fecha[k]#Hallando la fecha del día en el que v es X (es decir el día en el que sí hay alerta)
        variableprecipitacion=precipitacion[k]#Hallando el nivel de precipitación del día que v es X (es decir el día en el que sí hay alerta)
        print((datetime.utcfromtimestamp(dia).strftime('%Y-%m-%d %H:%M:%S'))+ " Alerta precipitación "+ str(variableprecipitacion))#Se imprime la fecha de la alerta, el tipo de alerta y el nivel de precipitación

#Hallando información de día en los que hay alerta de temperatura máxima
alrtatempmax=data.get("alertTmpMax")#Obteniendo diccionario alertTmpMax dentro del diccionario
tempmax=data.get("tmpMax")#Obteniendo diccionario tmpMax dentro del diccionario

for k,v in alrtatempmax.items():#Recorriendo pares llave:valor
    if v=="X":#Si el valor es X en el día
        fecha=data.get("dt")# Obteniendo diccionario "dt" dentro del diccionario
        dia=fecha[k]# Hallando la fecha del día en el que v es X (es decir el día en el que sí hay alerta)
        variabletempmax=tempmax[k] #Hallando la temperatura máxima del día que v es X 
        print((datetime.utcfromtimestamp(dia).strftime('%Y-%m-%d %H:%M:%S'))+ " Alerta temperatura máxima "+ str(variabletempmax))#Se imprime la fecha de la alerta, el tipo de alerta y la temperatura máxima

#Hallando información de día en los que hay alerta de temperatura mínima
alrtatempmin=data.get("alertTmpMin")#Obteniendo diccionario alerTmpMin dentro del diccionario
tmpMin=data.get("tmpMin")#Obteniendo diccionario tmpMin dentro del diccionario
for k,v in alrtatempmin.items():#Recorriendo pares llave:valor
    if v=="X":#Si el valor es X en el día
        fecha=data.get("dt")#Obteniendo diccionario "dt" dentro del diccionario
        dia=fecha[k]# Hallando la fecha del día en el que v es X (es decir el día en el que sí hay alerta)
        variabletempmin=tmpMin[k] #Hallando la temperatura mínima del día que v es X 
        print((datetime.utcfromtimestamp(dia).strftime('%Y-%m-%d %H:%M:%S'))+ " Alerta temperatura mínima "+ str(variabletempmin))#Se imprime la fecha de la alerta, el tipo de alerta y la temperatura mínima

# Hallando información de día en los que hay alerta de velocidad del viento
alrtavelviento=data.get("alertVelViento")# Obteniendo diccionario alertVelViento dentro del diccionario
velViento=data.get("velViento")#Obteniendo diccionario velViento dentro del diccionario
for k,v in alrtavelviento.items():#Recorriendo pares llaves:valor
    if v=="X":#Si el valor es X en el día
        fecha=data.get("dt")#Obteniendo diccionario "dt" dentro del diccionario
        dia=fecha[k]# Hallando la fecha del día en el que v es X (es decir el día en el que sí hay alerta)
        variableVelViento=velViento[k]#Hallando la velocidad del viento del día que v es X
